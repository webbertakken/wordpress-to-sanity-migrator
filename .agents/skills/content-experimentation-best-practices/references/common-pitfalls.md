...
**Why it's wrong:** Statistical significance fluctuates. At any point during a test, you might see
"significance" that disappears with more data. This is called the "peeking problem" or "repeated
...
**Why it's wrong:** You'll conclude "no difference" when there actually is one—you just couldn't
detect it.
...
**Why it's wrong:** You won't know which change caused the result. And each variable multiplies
...
**The problem:** Implementing a change because it was "statistically significant" even though the
effect was tiny.

**Why it's wrong:** Statistical significance ≠ practical significance. A 0.01% improvement isn't
...
**The problem:** The actual traffic split doesn't match the intended split (e.g., you expect 50/50
but observe 52/48).

**Why it's wrong:** SRM is a strong signal of an implementation bug — broken randomization, bot
contamination, or redirect issues. Results from experiments with SRM cannot be trusted.
...
**Why it's wrong:** Short experiments may show inflated effects that don't persist. Returning users
may click more simply because something looks new.
