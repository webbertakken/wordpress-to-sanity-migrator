...
Structured data helps search engines and AI understand your content. JSON-LD is the recommended
format.
...
Real-world pages often need multiple schema types. Use `@graph` to combine them. The `@context` is
defined once at the top level — omit it from individual schema generators when used inside `@graph`:
