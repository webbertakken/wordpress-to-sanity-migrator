...
description:
  SEO best practices for Sanity with Next.js, including metadata, Open Graph, sitemaps, redirects,
...
- **Smart defaults with optional overrides** — Don't require SEO fields; use existing content as
  fallback
...
**Key principle:** `seo.title` will never be `null` — it contains either the SEO override, the page
...
**Critical:** Always set `stega: false` when fetching for metadata. Stega characters in `<title>`
destroy SEO.
...
Use as fallback in metadata:
`url: page.seo.image ? urlFor(page.seo.image).url() : \`/api/og?id=\${page.\_id}\``
