...
description:
  Comprehensive guide for Sanity Visual Editing, including Presentation Tool, Stega (Content Source
  Maps), and Overlays.
---
...
The Studio plugin (`sanity/presentation`) that renders your front-end application inside an iframe
in the Studio. It enables the "Edit" overlay and bidirectional navigation.

### Content Source Maps (Stega)

Invisible characters embedded in strings that tell the Presentation Tool which field in which
document the content comes from.
...
When Visual Editing is enabled, string fields will contain invisible characters. You **MUST** clean
them before using the value for logic.
...
**Never** expose tokens in client bundles. Pass to `defineLive` for server/browser use only when
...
Show where documents appear in the front-end — enables quick navigation between Structure and
Presentation tools.
...
**NEVER** allow Stega strings in `<head>` tags (Title, Description, Canonical URLs). It destroys SEO
rankings and looks broken in search results.
...
For arrays (e.g., "Related Posts"), enable drag-and-drop in the preview using `data-sanity`
attributes and `useOptimistic`:
...
By default, editing a field in the Presentation Tool triggers a full page re-render. For pages with
many components, this can feel sluggish. **Presentation queries** solve this by fetching only the
specific block being edited.
...
This pattern works for both Page Builder blocks (`pageBuilder[]`) and Portable Text blocks
(`body[]`).
