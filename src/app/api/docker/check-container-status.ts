  ...
  return new Promise((resolve) => {
    exec('docker info', (err, stdout, stderr) => {
      if (!err) {
        ...
        if (errorOutput.includes('permission denied') || errorMessage.includes('permission denied')) {
          error = 'Permission denied accessing Docker'
          guidance = 'You may need to run this command with elevated permissions or add your user to the docker group.\n' +
                    'On Linux: sudo usermod -aG docker $USER (then log out and back in)\n' +
                    'On macOS/Windows: Ensure Docker Desktop is running'
        } else if (errorOutput.includes('cannot connect to the docker daemon') || 
                   errorMessage.includes('docker daemon') ||
                   errorOutput.includes('is the docker daemon running')) {
          error = 'Docker Desktop is not running'
          guidance = 'Docker Desktop is not running. Please start Docker Desktop and try again.\n' +
        ...
        } else if (errorOutput.includes('command not found') || errorMessage.includes('command not found')) {
          error = 'Docker is not installed'
          guidance = 'Docker does not appear to be installed on your system.\n' +
