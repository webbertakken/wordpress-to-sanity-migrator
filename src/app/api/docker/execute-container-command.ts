  ...
  try {
    if (command === 'start') {
      ...
      try {
        ...
        let errorDetails: any = getErrorDetails(err)
        ...
        if (err instanceof Error) {
          ...
          if (stderr.includes('bind: address already in use') || errMsg.includes('port is already allocated')) {
            errorMessage = 'Port 3306 is already in use'
            errorDetails = {
              ...errorDetails,
              guidance: 'Another application or container is already using port 3306.\n' +
            ...
            errorDetails = {
              ...errorDetails,
              guidance: 'A container named "' + CONTAINER_NAME + '" already exists.\n' +
