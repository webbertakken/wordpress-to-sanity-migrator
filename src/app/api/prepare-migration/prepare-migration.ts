...
function buildWordPressPageHierarchy(
  ...
  childPages.forEach((child) => {
    ...
    if (parent) {
      onProgress?.({
        step: 'page-hierarchy',
        message: `  └─ "${child.post_title}" is child of "${parent.post_title}"`,
      })
    } else {
      onProgress?.({
        step: 'page-hierarchy',
        message: `  └─ "${child.post_title}" has missing parent ID: ${child.post_parent}`,
      })
    }
  })
}
...
export async function prepareMigration(
  ...
  try {
    onProgress?.({
      step: 'connecting',
      message: 'Connecting to WordPress database...',
      progress: 10,
    })
    ...
    onProgress?.({
      step: 'fetching',
      message: 'Fetching WordPress posts and pages...',
      progress: 15,
    })
    ...
    onProgress?.({
      step: 'found-content',
      message: `Found ${typedContent.length} items to migrate`,
      progress: 20,
    })
    ...
    onProgress?.({
      step: 'processing',
      message: `Processing ${typedContent.length} items (${posts.length} posts, ${pages.length} pages)...`,
      progress: 25,
    })
    ...
    for (let i = 0; i < typedContent.length; i++) {
      ...
      if (mappedMediaRefs.length > 0) {
        onProgress?.({
          step: 'media-processing',
          message: `  - Found ${mappedMediaRefs.length} media references (${itemStats.totalFound} found, ${itemStats.totalMissing} missing)`,
        })

        // Collect missing media files
        mappedMediaRefs
          .filter((ref) => !ref.found)
          .forEach((ref) => {
      ...
      // The content has already been built above
    ...
    onProgress?.({
      step: 'media-summary',
      message: `- Found locally: ${totalMediaStats.totalFound}`,
    })
    ...
    if (!dryRun) {
      ...
      onProgress?.({
        step: 'file-written',
        message: `Migration data written to ${outputPath}`,
        progress: 90,
      })
    } else {
      onProgress?.({
