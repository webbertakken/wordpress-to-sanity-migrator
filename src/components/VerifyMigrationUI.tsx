  ...
  processedContent = processedContent.replace(
    ...
    (match: string, before: string, src: string, after: string) => {
      ...
      const mediaRef = mediaRefs.find((ref) => {
        ...
        return (
          ref.localPath &&
          (src === ref.localPath || src.endsWith(ref.localPath) || ref.localPath.endsWith(src))
        )
      })
...
export const VerifyMigrationUI: React.FC = () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            data.error ||
              data.details ||
              `Failed to load migration data (Status: ${response.status})`,
          )
        }
      ...
      filtered = filtered.filter((record) => {
        ...
        const content =
          record.transformed._type === 'post'
            ...
            : (record.transformed as SanityPageContent).heading +
              ' ' +
              ((record.transformed as SanityPageContent).subheading || '')
  ...
  const totalImages = records.reduce(
    (sum, r) => sum + r.transformed.media.filter((m) => m.type === 'image').length,
    0,
  )
  const totalAudio = records.reduce(
    (sum, r) => sum + r.transformed.media.filter((m) => m.type === 'audio').length,
    0,
  )
  const totalVideo = records.reduce(
    (sum, r) => sum + r.transformed.media.filter((m) => m.type === 'video').length,
    0,
  )
  const foundMedia = records.reduce(
    (sum, r) => sum + r.transformed.media.filter((m) => m.found).length,
    0,
  )
              ...
              >
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-300">
                    ...
                    <span>
                      {new Date(
                        record.transformed._type === 'post'
                          ? (record.transformed as SanityPostContent).date || ''
                ...
                <div className="mb-4 text-sm text-gray-300">
                  <div className="flex gap-4">
                    <span>
                      <strong>Slug:</strong> {record.transformed.slug.current}
                    </span>
                    {record.transformed._type === 'post' &&
                      (record.transformed as SanityPostContent).excerpt && (
                        <span>
                          <strong>Excerpt:</strong>{' '}
                          {(record.transformed as SanityPostContent).excerpt?.substring(0, 100)}...
                        </span>
                      )}
                    {record.transformed._type === 'page' &&
                      (record.transformed as SanityPageContent).subheading && (
                        <span>
                          <strong>Subheading:</strong>{' '}
                          {(record.transformed as SanityPageContent).subheading?.substring(0, 100)}
                          ...
                        </span>
                      )}
                  </div>
                    ...
                    <div className="bg-gray-700 rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 text-sm">
                        <div>
                          ...
                          <span className="ml-2 font-semibold">
                            {(() => {
                              const content =
                                record.transformed._type === 'post'
                                  ? (record.transformed as SanityPostContent).content
                                  : undefined
                        ...
                        <div>
                          ...
                          <span className="ml-2 font-semibold">
                            {record.transformed._type === 'post'
                              ? (record.transformed as SanityPostContent).content?.length || 0
                              : 0}
                          </span>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <span className="ml-2 font-semibold">
                            {record.transformed.media.filter((m) => m.type === 'image').length}
                          </span>
                        </div>
                        <div>
                          ...
                          {record.transformed.media.map((media, mediaIndex) => (
                            ...
                            >
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <span
                                  className={`px-2 py-1 rounded text-xs ${media.found ? 'bg-green-600' : 'bg-red-600'}`}
                                >
                      ...
                      <div>
                        ...
                        <div className="bg-gray-950 text-gray-100 p-3 rounded max-h-96 overflow-y-auto">
                          <pre className="whitespace-pre-wrap break-words text-sm">
                            ...
                            'post_content' in record.original &&
                            typeof (record.original as { post_content: string }).post_content ===
                              'string'
